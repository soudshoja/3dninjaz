...
| 260525-xbb | Per-SKU (Select-option) shipping weight — optional `weight` (g) on each Select option + admin Weight(g) input; Tier 0 weight resolution in quoteForCart (checkout+POS) & sumOrderWeight (admin booking); server re-reads from DB (T-17-09). Fixes 1kg-fallback wrong shipping. Generic across all products + future-proof. tsc clean, 15/15 tests. Branch `feat/per-sku-shipping-weight` (off origin/dev), not yet pushed. | 2026-05-25 | 497fefb..20113be | [260525-xbb-add-per-sku-select-option-shipping-weigh](./quick/260525-xbb-add-per-sku-select-option-shipping-weigh/) |
