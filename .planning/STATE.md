...
Status: c118ae6 on dev. CI auto-deploying. Server-side log monitor LIVE (1-min cron WhatsApp alerts to +96599800027). Awaiting human smoke on app.3dninjaz.com
Last activity: 2026-06-01 — Completed quick task 260601-9tj: mobile sticky live-preview strip on configurable/keychain PDP
...
| 260601-9tj | Mobile-only sticky live-preview strip on configurable/keychain PDP — pins a shrunk `KeychainPreview` (same live colour/text state) at `top-16 z-30` below the nav once editing starts, so customers stop scrolling up↔down between the form and the hero preview. Gated `touched && productType !== 'vending'` (vending preview is a tall 500×800 illustration). Added `preview-strip-in` keyframe (+ reduced-motion). Desktop + all pricing/cart logic untouched. tsc clean. Local visual run BLOCKED by OneDrive `.next` write failure (os error 362) — verify on dev deploy. Branch `fix/order-tracking-map-eta-label`. | 2026-06-01 | (see commit) | [260601-9tj-mobile-sticky-preview-strip](./quick/260601-9tj-mobile-sticky-preview-strip/) |
