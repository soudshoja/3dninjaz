...
**Pattern:** Client renders PayPal button → calls `/api/orders/create` → PayPal redirects → calls `/api/orders/capture/[id]` → server validates and marks order complete in DB.
...
**Upgrade path:** If Cloudinary free tier is exhausted, the $89/mo Plus tier or Cloudflare Images ($5/mo + $1/1000 served) are the next options.
...
**For v1 MVP:** Zustand for cart only. Skip TanStack Query — Server Components + server actions cover product listing and order management without client-side fetching.
...
**Alternative deployment if Vercel costs become an issue:** Railway — container-based, $5-15/month predictable pricing, includes managed PostgreSQL. Switch from Neon to Railway Postgres at that point. Railway is the better long-term cost play if traffic grows; Vercel is the faster zero-config launch option.
...
```bash
# Scaffold
npx create-next-app@latest printninjaz --typescript --tailwind --app --src-dir
...
# Forms & validation
npm install react-hook-form zod @hookform/resolvers drizzle-zod
