...
**Current preview:** `https://3dninjaz.com/v1` (Node app on 127.0.0.1:3100, LSWS reverse-proxy)
...
### 7. Document root swap (coordinate with D-07)

- [ ] Decide strategy: either (a) flip `public_html/` to serve the
      Node app's public assets via LSWS userdata config change, or
      (b) relocate Node to the root path and update userdata config to
      `ProxyPass /` rather than `ProxyPass /v1`.
- [ ] Back up current `public_html/` (coming-soon) via cPanel File
      Manager → Compress → Download before touching anything.
- [ ] Rename `public_html/` → `public_html_old/` as the rollback copy.
- [ ] Flip the domain root pointer per chosen strategy.
- [ ] Rebuild Apache conf: `/scripts/rebuildhttpdconf && /scripts/restartsrv_httpd`
      (root-level command; coordinate with hosting).
...
- [ ] `curl https://3dninjaz.com/robots.txt` → 200 with the rules
      generated by `src/app/robots.ts` (allow public routes; disallow
      /admin, /api, /bag, /checkout, /orders, auth routes; sitemap
      pointer).
- [ ] `curl https://3dninjaz.com/sitemap.xml` → 200, valid XML, lists
      6 static routes + all active products + categories.
...
- [ ] Google Search Console property verified for `3dninjaz.com` (or
      `sc-domain:3dninjaz.com`).
- [ ] Sitemap submitted at Search Console → Index → Sitemaps →
      `https://3dninjaz.com/sitemap.xml`.
- [ ] Requested indexing for `/` via the URL inspection tool.
...
_Last updated: 2026-04-19 (Plan 04-04 execution)_
